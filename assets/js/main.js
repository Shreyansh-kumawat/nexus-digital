...
(function initHero3D(){
  ...
  const dl = new THREE.DirectionalLight(0x5b5ef4,1.2); dl.position.set(5,5,5); scene.add(dl);
  const dl2 = new THREE.DirectionalLight(0xf43f5e,0.4); dl2.position.set(-5,-3,2); scene.add(dl2);
  const shapes = [];
  ...
  function animate(){
    ...
    shapes.forEach(s=>{ s.mesh.rotation.x+=s.speed.x; s.mesh.rotation.y+=s.speed.y; s.mesh.position.y+=Math.sin(t+s.float.offset)*0.002*s.float.amp*5; });
    camera.position.x+=(mx-camera.position.x)*0.04; camera.position.y+=(-my-camera.position.y)*0.04;
...
(function initTech3D(){
  ...
  // FIX 1: alpha:true + clearColor alpha=0 — removes white flash/box
  const renderer = new THREE.WebGLRenderer({canvas, alpha:true, antialias:true});
  ...
  const key = new THREE.DirectionalLight(0xffffff,1.6); key.position.set(10,10,10); scene.add(key);
  const fill = new THREE.DirectionalLight(0x5b5ef4,0.8); fill.position.set(-8,-5,5); scene.add(fill);
  const rim = new THREE.DirectionalLight(0xa5b4fc,0.5); rim.position.set(0,5,-10); scene.add(rim);
  ...
  // FIX 2: GLTFLoader CDN exposes standalone global 'GLTFLoader', not 'THREE.GLTFLoader'
  let eyeModel=null;
  ...
  if(LoaderClass){
    ...
    loader.load('./assets/3d/blue_eyeball_free.glb', function(gltf){
      ...
      scene.add(eyeModel);
      console.log('%c Eye model loaded!', 'color:#5b5ef4;font-weight:bold');
    }, undefined, err=>console.warn('GLB error:',err));
  } else {
    console.warn('GLTFLoader not found on page');
  }
  ...
  function animate(){
    ...
    if(eyeModel){ eyeModel.rotation.y=t*0.3; eyeModel.position.y=Math.sin(t*0.6)*0.35; }
    renderer.render(scene,camera);
  }
  animate();
